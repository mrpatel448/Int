  ...
  delete(id: number) {
    if (confirm('Are you really want to delete this record ?')) {
      this.empService.deleteEmployee(id).subscribe(data => {
        this.empService.getEmployees().subscribe(data => {
          this.empService.listEmployee = data;
          this.toastr.error('Success', ' Record Deleted Successfully');
        })
        //alert('Record deleted successfully... from emp-detail-compo.ts');
        //console.log('Record deleted successfully... from emp-detail-compo.ts');
      },
        err => {
          //alert('Something went wrong, record not deleted...');
          //console.log('Something went wrong, record not deleted...')
        });
    }
  }
}
